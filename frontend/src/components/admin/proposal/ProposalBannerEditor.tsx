...
import type { ProposalBannerFields } from '@/types'

const PRESET_COLORS = ['#5f5248', '#faf9f7', '#c9a96e', '#f3f0eb', '#1c1714', '#ffffff']

interface ProposalBannerEditorProps {
  role: 'header' | 'footer'
  value: ProposalBannerFields
  onChange: (value: ProposalBannerFields) => void
}

export default function ProposalBannerEditor({ role, value, onChange }: ProposalBannerEditorProps) {
  const label = role === 'header' ? 'Letterhead' : 'Footer'
  const textPlaceholder = role === 'header'
    ? 'A title or greeting shown at the top of the proposal…'
    : 'A closing note, address, or contact info shown at the bottom…'

  const set = (patch: Partial<ProposalBannerFields>) => onChange({ ...value, ...patch })
